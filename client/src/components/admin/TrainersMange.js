  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setLoading(false);
        console.log(userId)
        console.log("Type of userIdNumber:", typeof userId);
        const userIdNumber = Number(userId);

        console.log("response.data", response.data)
        if (userId) {
          console.log("userId",userId)
          // iterate over the users array to find the user with the matching ID
          const user = response.data.find(user => user.user_id === userIdNumber);
          console.log("1242141242142142142112", user)
          if (user) {
